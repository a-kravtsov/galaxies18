...
# inherit class for the homework 4 from the base model_galaxy class
# add the required functions to it
from colossus.lss import peaks

class gmodel_heating(model_galaxy):
...
class gmodel_wind(gmodel_r50):
    ...
    def Muratov15modified(self):
        #if self.Ms > 0 and self.Ms <= 5.e9:
        #    return np.maximum(0.,self.etawind*(self.Ms/1.e10)**(-0.35)-3.)
        #else:
        #    return 0.
        if self.Ms > 0: 
            return np.maximum(0.,self.etawind*(self.Ms/1.e10)**(-0.35)-4.6)
        else:
            return 0.
...
class gmodel_H2(gmodel_wind):
    ...
    def f_H2(self, Sigmag, ZZsun):
        ...
        tc = 3.34e-7* Sigmag * ZdZsun # assumes concentration c=5
    ...
    def M_H2(self,t):
        ...
        rd = self.R_d(t)
        rg = np.linspace(0., 6.*rd, 20) 
        Sigma0 = self.Mg/(2.*np.pi*rd**2)
        sgd = Sigma0*np.exp(-rg/rd)
        sgd = rg * sgd * self.f_H2(sgd, ZZsun) #self.f_H2(sgd,ZZsun)
